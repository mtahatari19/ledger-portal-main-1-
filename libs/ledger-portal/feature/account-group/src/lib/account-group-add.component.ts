...
export class AccountGroupAddComponent {
  ...
  creatingAccountGroup = signal(false);
  ...
  onSubmit(): void {
    ...
    this.creatingAccountGroup.set(true);
    ...
    this.accountGroupService
      ...
      .pipe(finalize(() => this.creatingAccountGroup.set(false)))
      .subscribe({
        next: () => {
          this.alert.open('گروه حساب جدید با موفقیت ایجاد شد');
          this.router.navigate(['/console/account-group/list']);
        },
        error: () => {
          this.alert.open('خطا در ایجاد گروه حساب');
        },
      });
  }
